...
export const createQuest = async (req, res) => {
  try {
    ...
    const rewards = RPG.getDifficultyRewards(
      difficulty,
      category
    );
    ...
    const newQuest = await dbService.createQuest({
      ...
      // Important: reward has not been claimed yet.
      rewardClaimed: false,
...
export const updateQuest = async (req, res) => {
  try {
    ...
    const updatedQuest = await dbService.updateQuest(
      id,
      updates
    );
...
export const deleteQuest = async (req, res) => {
  try {
    ...
    const deleted = await dbService.deleteQuest(
      id,
      userId
    );
...
export const completeQuest = async (req, res) => {
  try {
    ...
    /*
     * If the quest is already completed, allow the UI
     * to toggle it back to incomplete.
     *
     * IMPORTANT:
     * rewardClaimed remains true.
     *
     * Therefore completing it again will NOT award
     * XP/coins/stats a second time.
     */
    ...
    /*
     * A quest that was already rewarded must not
    ...
    if (quest.rewardClaimed) {
      ...
      return res.json({
        ...
        message: 'Quest completed. Reward already claimed.'
      });
    }

    /*
     * First completion:
     * Apply XP, coins, streak and attribute rewards.
     */
    const {
      updatedUser,
      rewards
    } = RPG.applyQuestCompletion(
      req.user,
      quest
    );
     ...
     */
    const savedUser = await dbService.updateUser(
      userId,
      updatedUser
    );
    ...
    if (!savedUser) {
      return res.status(500).json({
        success: false,
        message: 'Failed to save player progress.'
      });
    }

    /*
     * Mark quest as completed AND permanently record
     * that its reward has been claimed.
     */
    const updatedQuest = await dbService.updateQuest(
      id,
      {
    ...
    res.json({
      ...
      message: rewards.didLevelUp
        ...
        : `Quest complete! +${rewards.xpGained} XP, +${rewards.coinsGained} Coins!`
    });
  } catch (err) {
...
export const resetRecurringQuests = async (req, res) => {
  try {
    ...
    const quests = await dbService.getQuestsByUserId(
      userId
    );

    const recurringCompleted = quests.filter(
      q => q.isRecurring && q.isCompleted
    );

    for (const q of recurringCompleted) {
      await dbService.updateQuest(
        q._id.toString(),
        {
          isCompleted: false,
          completedAt: null,
          ...
          /*
           * Recurring quests get a fresh reward
           * on the next cycle.
           */
          rewardClaimed: false
        }
    ...
    const updatedQuests =
      await dbService.getQuestsByUserId(userId);
    ...
    console.error(
      'Reset recurring quests error:',
      err
    );

    res.status(500).json({
      success: false,
      message: 'Failed to refresh recurring quests.'
    });
  }
};